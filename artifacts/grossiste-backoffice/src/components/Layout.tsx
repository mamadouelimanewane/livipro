...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      {mobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-40 bg-navy/80 backdrop-blur-sm" onClick={() => setMobileMenuOpen(false)} />
      )}
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        {/* Mobile Topbar */}
        <header className="md:hidden h-16 bg-white border-b border-border flex items-center px-4 justify-between shrink-0 z-10">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-primary flex items-center justify-center">
              <img src={`${import.meta.env.BASE_URL}images/logo.png`} alt="Logo" className="w-5 h-5 brightness-0 invert" />
            </div>
            <span className="font-bold text-navy">LiviPro B2B</span>
          </div>
